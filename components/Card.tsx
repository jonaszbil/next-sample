...
const Card: FC<{ title: string; text: string; color: string }> = ({ title, text, color }) => {
  ...
  return (
    <div className={`${color} h-[32rem] w-80 p-12 text-white flex flex-col justify-between`}>
      <div>
        <div className="flex justify-center items-center mb-9 aspect-square w-10 rounded-full bg-black bg-opacity-20"/>
        <h2 className="mb-6 font-big-shoulders text-5xl font-bold">{title}</h2>
        <p className="font-lexend text-[14px] leading-6 opacity-75">{text}</p>
      </div>
